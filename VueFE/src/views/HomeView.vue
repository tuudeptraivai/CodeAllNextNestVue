    ...
    <main v-else class="min-h-screen pt-20 overflow-x-hidden">
      ...
      <section class="relative w-full h-[70vh] md:h-[85vh] group">
        <Swiper
          ...
          class="w-full h-full"
        >
          <SwiperSlide v-for="movie in heroMovies" :key="movie.id || movie._id">
        ...
        <div class="absolute bottom-12 right-6 md:right-16 z-20 w-full max-w-[400px] hidden md:block">
          <Swiper
            ...
            class="thumbs-swiper"
          >
            <SwiperSlide v-for="movie in heroMovies" :key="movie.id || movie._id" class="cursor-pointer opacity-40 hover:opacity-100 transition-all [&.swiper-slide-thumb-active]:opacity-100 [&.swiper-slide-thumb-active]:ring-2 [&.swiper-slide-thumb-active]:ring-primary rounded-lg overflow-hidden border border-white/20">
